...
// Driver info comes from the order API response
...
export default function OrderTrackingPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { getOrder, fetchOrder, orderJustPlaced, clearOrderJustPlaced, updateOrderInState } = useApp();

  const [localOrder, setLocalOrder] = useState<Order | null>(null);
  const [loading, setLoading] = useState(true);

  // First try to get from context, else fetch from API
  const contextOrder = getOrder(id);

  const order = contextOrder ?? localOrder;

  // Fetch order from API if not in context
  useEffect(() => {
    let cancelled = false;
  ...
  const handleWsMessage = useCallback(
    (data: { type: string; order: any }) => {
      if (data.type === "order_updated" && data.order && data.order.id === id) {
        setLocalOrder(data.order);
        updateOrderInState(data.order);
      }
    },
    [id, updateOrderInState]
  );

  useOrderSocket(handleWsMessage);

  // Driver name from order API data
  const driverName = useMemo(() => {
    if (!order) return "Delivery Partner";
    const o = order as unknown as { driverName?: string };
    return o.driverName || "Delivery Partner";
  }, [order]);
  ...
  const showConfirmation = orderJustPlaced === id;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-4 text-gray-500 dark:text-zinc-500">Loading order...</p>
      </div>
    );
  }
    ...
    <div>
      ...
      {showConfirmation && (
        <OrderConfirmation
          ...
          estimatedTime={eta}
          onDismiss={clearOrderJustPlaced}
        />
      )}
      ...
      <div className="mt-4 flex items-center gap-2 rounded-lg bg-gray-50 px-4 py-2.5 text-sm dark:bg-zinc-800/50">
        ...
        <span className="font-medium text-gray-900 dark:text-zinc-100">
          {(order as unknown as { deliveryAddress?: string }).deliveryAddress || order?.restaurantAddress || "Your location"}
        </span>
      </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div className="mt-4 border-t border-gray-100 pt-3 dark:border-zinc-800">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500 dark:text-zinc-500">Payment</span>

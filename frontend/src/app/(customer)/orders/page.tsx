...
export default function OrdersPage() {
  const { orders: liveOrders, refreshOrders, ordersLoading, isAuthenticated } = useApp();
  ...
  const allOrders: DisplayOrder[] = useMemo(() => {
    return liveOrders.map((o) => ({
      ...
      date: o.date,
      rating: o.rating,
      isLive: activeStatuses.has(o.status),
    }));
  }, [liveOrders]);
  ...
  if (!isAuthenticated) {
    return (
      <EmptyState
        ...
        description="Sign in to view your order history and track active orders"
    ...
    <div>
      ...
      {!ordersLoading && filtered.length === 0 && (
        <EmptyState
          ...
          description={filter === "active" ? "You don't have any active orders right now" : "Start ordering from your favorite restaurants!"}
          actionLabel="Browse Restaurants"
          actionHref="/restaurants"
        />
      )}

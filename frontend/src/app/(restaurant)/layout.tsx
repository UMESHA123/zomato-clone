...
export default function RestaurantLayout({
  ...
  useEffect(() => {
    // Fetch restaurant info for the logged-in owner
    const token = localStorage.getItem("zomato_token");
    const userStr = localStorage.getItem("zomato_user");
      ...
      fetch(`http://localhost:8080/api/restaurants/owner/${ownerId}`)
        ...
        .then((data) => {
          if (Array.isArray(data) && data.length > 0) {
            setRestaurantName(data[0].name || user.name || "My Restaurant");
            setRestaurantAddress(data[0].address || "");
          } else if (user.name) {
      ...
      >
            ...
            <div className="flex items-center gap-3">
              <span
                className={`inline-block h-3 w-3 rounded-full ${
                  isOpen ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]" : "bg-red-500"
                }`}
              />
        ...
        <div className="border-b border-gray-200 px-6 py-4 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-zinc-100">{restaurantName}</p>
              {restaurantAddress && <p className="text-xs text-gray-500 dark:text-zinc-500">{restaurantAddress}</p>}
            </div>
          </div>
        </div>
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                {link.href === "/dashboard/orders" && pendingOrders > 0 && (
        ...
        <div className="border-t border-gray-200 px-6 py-4 dark:border-zinc-800">
          <Link
            href="/"
            className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm text-gray-500 transition-colors hover:bg-gray-50 hover:text-gray-900 dark:text-zinc-500 dark:hover:bg-zinc-800 dark:hover:text-zinc-100"
          >

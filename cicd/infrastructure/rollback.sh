...
#
# Usage:
#   ./rollback.sh <service-name> <previous-image-tag>
#   ./rollback.sh user-service 41-def5678
...
LOG_FILE="${LOG_DIR}/rollback-$(date +%Y%m%d).log"
...
# ---- Parse args ----
SERVICE="${1:?Usage: ./rollback.sh <service-name> <image-tag>}"
ROLLBACK_TAG="${2:?Usage: ./rollback.sh <service-name> <image-tag>}"
...
# ---- Confirmation ----
echo ""
echo "WARNING: This will rollback ${SERVICE} to tag ${ROLLBACK_TAG}"
echo ""
...
if wait_for_healthy "${SERVICE}"; then
    log ""
    log "Rollback successful! ${SERVICE} is now running ${ROLLBACK_TAG}"
else
    log_error ""
